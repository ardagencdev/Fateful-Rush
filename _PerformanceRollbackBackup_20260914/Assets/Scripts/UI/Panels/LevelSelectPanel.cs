...
public class LevelSelectPanel : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // objects instead of Destroy/Instantiate spikes in the middle of a slide.
        EnsureButtonPool(Mathf.Max(1, levelsPerPage));
        ClearCreatedButtons();
    ...
    private void CreateCurrentPageButtons()
    {
        ...
        {
            LevelButtonUI button = createdButtons[i];
            if (button == null)
                continue;
    ...
    private void EnsureButtonPool(int requiredCount)
    {
        ...
        while (createdButtons.Count < requiredCount)
        {
            LevelButtonUI button =
                Instantiate(levelButtonPrefab, levelButtonsContainer);
    ...
    {
        // Keep instantiated level buttons as a small reusable pool.
        // Disabling is enough; destroying/recreating them during every page
        // transition can trigger GC and Canvas rebuild spikes.
        for (int i = 0; i < createdButtons.Count; i++)
        {
            LevelButtonUI button = createdButtons[i];
        ...
        {
            LevelButtonUI button = existingButtons[i];
