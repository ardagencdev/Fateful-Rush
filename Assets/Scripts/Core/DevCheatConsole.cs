...
public sealed class DevCheatConsole : MonoBehaviour
{
    ...
    private const float MobileTapWindow = 1.35f;
    ...
    private void HandleMobileOpenGesture()
    {
        ...
        bool insideHotspot =
            position.x <= Screen.width * MobileHotspotWidthRatio &&
            position.y >= Screen.height * (1f - MobileHotspotHeightRatio);
        ...
        if (now - lastMobileTapTime > MobileTapWindow)
            mobileTapCounter = 0;

        lastMobileTapTime = now;
        mobileTapCounter++;

        if (mobileTapCounter < MobileTapCount)
            return;
